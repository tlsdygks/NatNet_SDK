...
// #define UDP_IP "10.42.0.63"
// #define UDP_PORT 9763
...
//     if (std::abs(sinp) >= 1)
//         *pitch = std::copysign(M_PI / 2, sinp); // 범위를 벗어나는 경우, 90도로 설정
//     else
//         *pitch = std::asin(sinp);
...
// // UDP 소켓 설정 함수
// int set_udp_socket(const char *ip, int port, struct sockaddr_in *dest_addr) {
...
//     return sockfd;
// }
// void calculate_velocity(float x, float y, float z, 
//                         float prev_x, float prev_y, float prev_z, 
//                         uint64_t curr_time, uint64_t prev_time, 
//                         float* vx, float* vy, float* vz) {
//     float dt = (curr_time - prev_time) / 1e6;  // 마이크로초에서 초로 변환
//     if (dt > 0) {
//         *vx = (x - prev_x) / dt;
//         *vy = (y - prev_y) / dt;
//         *vz = (z - prev_z) / dt;
...
// // void send_mavlink_vision_position_estimate(int sockfd, struct sockaddr_in *dest_addr, float x, float y, float z, float roll, float pitch, float yaw) {
// //     mavlink_message_t msg;
// //     mavlink_vision_position_estimate_t vision_position_estimate;

// //     vision_position_estimate.usec = 0; // 시간 정보를 0으로 설정, 필요시 실제 시간 값을 넣을 수 있음
// //     vision_position_estimate.x = x;
// //     vision_position_estimate.y = y;
// //     vision_position_estimate.z = z;
// //     vision_position_estimate.roll = roll;
// //     vision_position_estimate.pitch = pitch;
// //     vision_position_estimate.yaw = yaw;

// //     mavlink_msg_vision_position_estimate_encode(3, 201, &msg, &vision_position_estimate);

// //     uint8_t buf[MAVLINK_MAX_PACKET_LEN];
// //     uint16_t len = mavlink_msg_to_send_buffer(buf, &msg);

// //     int ret = sendto(sockfd, buf, len, 0, (struct sockaddr *)dest_addr, sizeof(struct sockaddr_in));

// //     // printf("Send MAVLink message with ret = %d\n", ret);
// // }

// void send_mavlink_odometry(int sockfd, struct sockaddr_in *dest_addr, 
...
//     odometry.time_usec = 0;  // set appropriate timestamp
//     odometry.frame_id = MAV_FRAME_LOCAL_NED;
//     odometry.child_frame_id = MAV_FRAME_BODY_NED;
...
//     // Angular velocity
//     odometry.rollspeed = rollspeed;
//     odometry.pitchspeed = pitchspeed;
//     odometry.yawspeed = yawspeed;

//     // Encode and send the MAVLink message
//     mavlink_msg_odometry_encode(3, 201, &msg, &odometry);
//     uint8_t buf[MAVLINK_MAX_PACKET_LEN];
//     uint16_t len = mavlink_msg_to_send_buffer(buf, &msg);
//     sendto(sockfd, buf, len, 0, (struct sockaddr *)dest_addr, sizeof(struct sockaddr_in));
// }
...
// //     int udp_sockfd = set_udp_socket(UDP_IP, UDP_PORT, &dest_addr);
// //     if (udp_sockfd >= 0) {
// //         if (data->nRigidBodies > 0) {
// //             g_count++;
// //             if (g_count % 6 == 0) {
// //                 // 오일러 각을 계산하기 위해 쿼터니언을 변환
...
// //                 send_mavlink_vision_position_estimate(
// //                     udp_sockfd,
// //                     &dest_addr,
...
//     // Copy and calculate time deltas
//     uint64_t curr_time = data->TransmitTimestamp;
//     shared_ptr<sFrameOfMocapData> pDataCopy = make_shared<sFrameOfMocapData>();
//     NatNet_CopyFrame(data, pDataCopy.get());

//     // Calculate velocity
//     float vx, vy, vz;
//     // printf("%f, %f, %f \n",data->RigidBodies[0].x,data->RigidBodies[0].y,data->RigidBodies[0].z);
//     calculate_velocity(data->RigidBodies[0].x, data->RigidBodies[0].y, data->RigidBodies[0].z, 
//                        prev_x, prev_y, prev_z, curr_time, prev_time, &vx, &vy, &vz);
//     //printf("%f, %f, %f \n",vx,vy,vz);

//     // Calculate angular velocity from quaternion (this part can be more complex, but we can use finite differences)

//     // Update the previous position and time
//     prev_x = data->RigidBodies[0].x;
//     prev_y = data->RigidBodies[0].y;
//     prev_z = data->RigidBodies[0].z;
//     prev_time = curr_time;

//     // UDP socket setup and send odometry message
//     struct sockaddr_in dest_addr;
//     int udp_sockfd = set_udp_socket(UDP_IP, UDP_PORT, &dest_addr);
//     if (udp_sockfd >= 0) {
//         if (data->nRigidBodies > 0) {
...
//                     data->RigidBodies[0].qw,
//                     vx, vy, vz,
//                     0, 0, 0  // Placeholder for angular velocities
//                 );
//             }
//         }
//         close(udp_sockfd);
